...
async def execute_pipeline(
    ...
    max_depth = 20
    if specified_depths == "-1":
        depth_lst = list(range(max_depth, 0, -1))
    else:
        depth_lst = [int(depth) for depth in specified_depths.split(",")]
...
if __name__ == "__main__":
    ...
    depths_to_run = os.getenv("DEPTHS_TO_RUN", "-1")
    max_product_per_categories = int(os.getenv("MAX_PRODUCTS_PER_CATEGORY", "50000"))

    setup_logger()
    asyncio.run(execute_pipeline(max_product_per_categories, -1, depths_to_run))
